        ...
        <form action="{{ route('documents.update', $document) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <select name="idUtilisateur" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                    @foreach($utilisateurs as $u)
                        <option value="{{ $u->idUtilisateur }}"
                            {{ old('idUtilisateur', $document->idUtilisateur) == $u->idUtilisateur ? 'selected' : '' }}>
                            {{ $u->prenom }} {{ $u->nom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="idMatiere" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                        @foreach($matieres as $m)
                            <option value="{{ $m->idMatiere }}"
                                {{ old('idMatiere', $document->idMatiere) == $m->idMatiere ? 'selected' : '' }}>
                                {{ $m->nomMatiere }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="idNiveau" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                        @foreach($niveaux as $n)
                            <option value="{{ $n->idNiveau }}"
                                {{ old('idNiveau', $document->idNiveau) == $n->idNiveau ? 'selected' : '' }}>
                                {{ $n->nomNiveau }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2">{{ old('description', $document->description) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nouveau fichier (laisser vide pour conserver)</label>
                <p class="text-xs text-gray-500 mb-1">Fichier actuel : <span class="font-mono">{{ $document->fichier }}</span></p>
