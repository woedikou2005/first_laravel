...
<div class="max-w-md mx-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">🎓 Créer un niveau</h1>
    <div class="bg-white rounded-xl shadow p-6">
        <form action="{{ route('niveaux.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nomNiveau" value="{{ old('nomNiveau') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-400">
                @error('nomNiveau') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg font-medium transition">Créer</button>
